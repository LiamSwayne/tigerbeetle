...
pub fn ForestType(comptime _Storage: type, comptime groove_cfg: anytype) type {
    ...
    return struct {
        ...
        pub const Options = struct {
            ...
            pub const compaction_block_count_min: u32 = CompactionPipeline.forest_block_count_min;
        };
        ...
        pub fn compact(forest: *Forest, callback: Callback, op: u64) void {
            ...
            forest.compaction_pipeline.beat_start(op, compact_trees_callback);
        ...
        fn compact_finish(forest: *Forest) void {
            ...
            assert(forest.compaction_progress.?.trees_done);
            assert(forest.compaction_progress.?.manifest_log_done);
            assert(forest.compaction_pipeline.pool.idle());
            assert(forest.compaction_pipeline.pool.blocks_acquired() <=
                CompactionPipeline.forest_block_count_bar_max);
                    ...
                    for (0..constants.lsm_levels) |level_b| {
                        if (level_active(.{ .level_b = level_b, .op = op })) {
                            inline for (comptime std.enums.values(Forest.TreeID)) |tree_id| {
                                forest.compaction_pipeline.compaction_at(level_b, tree_id)
                                    .bar_complete();
                            }
                        }
                    }
...
fn CompactionPipelineType(comptime Forest: type, comptime Grid: type) type {
    return struct {
        ...
        // blocks reserved between the bars plus enough blocks to complete a single bar.
        const forest_compactions_concurrent_max = stdx.div_ceil(constants.lsm_levels, 2);
        ...
        pub fn init(
            ...
            assert(block_count >= CompactionPipeline.forest_block_count_min);
        ...
        fn beat_resume(self: *CompactionPipeline) void {
        ...
        fn beat_resume_callback(pool: *ResourcePool) void {
            const self: *CompactionPipeline = @fieldParentPtr("pool", pool);
            self.beat_resume();
        }

        fn beat_finish(self: *CompactionPipeline) void {
            assert(self.callback != null);
            self.grid.on_next_tick(beat_finish_next_tick, &self.next_tick);
        }

        fn beat_finish_next_tick(next_tick: *Grid.NextTick) void {
            const self: *CompactionPipeline = @alignCast(
