...
pub fn CompactionType(
    ...
    return struct {
        ...
        // split.
        //
        // When a block is read from disk, it is added to the tail of the corresponding queue.
        // When the head block from both level a and level b queues is in the .read_value_block_done
        // state, the two blocks are popped off the queues and passed down to merge. At this point,
        // any number of the blocks still in the queue can continue their read operations.
        //
        // For index blocks, queues of length one are used. Because an index block is freed as soon
        // as the read for the last value block is scheduled, the pipeline should be full even
        ...
        // - the blocks are not used after the completion of the IO.
        // That is, only the number of outstanding operations needs to be tracked. Use a RingBuffer
        // with void elements rather than an u32 for API uniformity and comptime upper bound.
        // We have <https://github.com/ziglang/zig/issues/3806> at home.
        ...
        pub fn idle(compaction: *Compaction) bool {
        ...
        pub fn bar_complete(compaction: *Compaction) void {
            ...
            defer assert(compaction.stage == .inactive);
        ...
        /// - reserve te grid blocks for the output.
